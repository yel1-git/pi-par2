...
%       -> Proc b (Su n) 
%       -> Proc c (Su n)
app_stream_bin(F, N, {Pid1, Sus1}, {Pid2, Sus2}) ->
        R1 = sync_stream3(Sus1, N),
        R2 = sync_stream3(Sus2, N),
        R3Sus = process(fun({X,Y}) -> F(X,Y) end),
        Inp  = lists:zip(R1, R2),
        app_stream(R3Sus, Inp).
...
app_stream_33(N, Sus, F) ->
    io:format("app_stream_3 received ~p ~n", [N]),
    R1 = sync_stream3(Sus, N),
    Sus2 = process(F),
    io:format("Passing to app_stream... ~n", []),
    app_stream(Sus2, R1).

app_fold(N, Sus, F, I) ->
    io:format("app_fold received ~p ~n", [N]),
    R1 = sync_stream3(Sus, N ),
