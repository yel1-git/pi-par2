-module(parSumEuler2_sync).
-compile(export_all).

%% Instrumented copy of generated_pl/parSumEuler2.erl -- see parCpi_sync.erl
...
run(Nw, Size) ->
    ...
    {BenchResult, SyncStats} = play_sync:profile(fun() ->
        sk_profile:benchmark(fun parSumEuler/2, [ChunkSize, List], 1)
    end),
    io:format("SumEuler ~p workers: ~p~n", [Nw, BenchResult]),
    io:format("SumEuler ~p workers sync stats: ~p~n", [Nw, SyncStats]).
